...
class Login extends CI_Controller {
	...
	public function loadPassword(){
		$email=$this->encryption->decrypt($_GET['XSRS']);
		
		$user=$this->Login_model->getUser("user_email",$email);
		$user=$user->row_array();
		$this->setSession($user);
		...
		if(isset($_POST['verifyPassword'])){
			...
			if($password==$repassword){
				 $hashedpass =  password_hash("$password", PASSWORD_BCRYPT);
					$this->Login_model->updateuser($hashedpass,1,$this->session->userdata('user_id'));
				header("Location: ../..");
			}
			}
		}
